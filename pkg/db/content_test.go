...
func TestContent(t *testing.T) {
	testsuite.ContentSuite(t, "metadata", func(ctx context.Context, root string) (context.Context, content.Store, func() error, error) {
		return createContentStore(ctx, root)
	})
}
...
func checkContentLeased(ctx context.Context, db *DB, dgst digest.Digest) error {
	...
	return db.View(func(tx *bolt.Tx) error {
		bkt := getBucket(tx, bucketKeyVersion, bucketKeyObjectLeases, []byte(lease), bucketKeyObjectContent)
		if bkt == nil {
...
func checkIngestLeased(ctx context.Context, db *DB, ref string) error {
	...
	return db.View(func(tx *bolt.Tx) error {
		bkt := getBucket(tx, bucketKeyVersion, bucketKeyObjectLeases, []byte(lease), bucketKeyObjectIngests)
		if bkt == nil {
			return fmt.Errorf("bucket not found %s: %w", lease, errdefs.ErrNotFound)
		}
		v := bkt.Get([]byte(ref))
