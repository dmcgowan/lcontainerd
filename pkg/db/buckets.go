...
func imagesBucketPath() [][]byte {
	return [][]byte{bucketKeyVersion, bucketKeyObjectImages}
}

func createImagesBucket(tx *bolt.Tx) (*bolt.Bucket, error) {
	return createBucketIfNotExists(tx, imagesBucketPath()...)
}

func getImagesBucket(tx *bolt.Tx) *bolt.Bucket {
	return getBucket(tx, imagesBucketPath()...)
}

func createBlobBucket(tx *bolt.Tx, dgst digest.Digest) (*bolt.Bucket, error) {
	bkt, err := createBucketIfNotExists(tx, bucketKeyVersion, bucketKeyObjectContent, bucketKeyObjectBlob)
	if err != nil {
		return nil, err
	}
...
func getBlobsBucket(tx *bolt.Tx) *bolt.Bucket {
	return getBucket(tx, bucketKeyVersion, bucketKeyObjectContent, bucketKeyObjectBlob)
}

func getBlobBucket(tx *bolt.Tx, dgst digest.Digest) *bolt.Bucket {
	return getBucket(tx, bucketKeyVersion, bucketKeyObjectContent, bucketKeyObjectBlob, []byte(dgst.String()))
}

func getIngestsBucket(tx *bolt.Tx) *bolt.Bucket {
	return getBucket(tx, bucketKeyVersion, bucketKeyObjectContent, bucketKeyObjectIngests)
}

func createIngestBucket(tx *bolt.Tx, ref string) (*bolt.Bucket, error) {
	bkt, err := createBucketIfNotExists(tx, bucketKeyVersion, bucketKeyObjectContent, bucketKeyObjectIngests, []byte(ref))
	if err != nil {
		return nil, err
	}
	return bkt, nil
...
func getIngestBucket(tx *bolt.Tx, ref string) *bolt.Bucket {
	return getBucket(tx, bucketKeyVersion, bucketKeyObjectContent, bucketKeyObjectIngests, []byte(ref))
}
